...
document.querySelector('input[name="link"]').addEventListener('input', function(){
  ...
  browser.runtime.onMessage.addListener(async function(message, sender) {
    if (message.to === "popup") {
      if(message.message.services && message.message.inputs){
        ...
        browser.storage.local.set({ auth: inputs});
        browser.storage.local.set({ services: res.data});
        services = res.data;
        setServiceRoot();
        authData = inputs;
        buttonAuth.innerText = 'Change';
        ...
        if('data' in res && res.data.order_id){
          browser.storage.local.get(['lists'], function(e){
            let lists = [];
            if(e.lists){
              lists = e.lists;
            }
            ...
            document.querySelector('#lists').classList.remove('none');
            browser.storage.local.set({lists}, function(){
  ...
  function renderList(){
    ...
    browser.storage.local.get(['lists'], function(data){
